...
// mas pensada para ser aberta a partir do QR Code do equipamento — sem
// exigir login. É por isso que este componente tem sua própria rota,
// fora do <ProtectedRoute> (veja App.jsx: /relatorio/:osId).
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!docOS.exists()) {
          setErro("Relatório de manutenção não encontrado.");
          setCarregando(false);
          return;
        }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50 p-6">
        <div className="w-full max-w-md rounded-xl border border-red-200 bg-red-50 p-6 text-center">
          <p className="text-lg font-semibold text-red-900">{erro || "Relatório não encontrado"}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="mx-auto w-full max-w-4xl">
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <h1 className="text-2xl font-bold text-gray-900">Relatório de Manutenção</h1>
          <div className="flex gap-2">
            {ordem.equipamentoId && (
              <button
                onClick={() => navigate(`/eq/${ordem.equipamentoId}`)}
